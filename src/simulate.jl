...
CONSUMPTION_PER_YEAR =   6440 # GWh per year
LIFE_TIME = 20                # lifetime of a turbine in years
YEARS = 20                    # simulation time in years
...
function calc_scenario(turbines_per_year)
    s = State(0, 0.0, 0.0, 0.0, 0.0)
    Y = zeros(YEARS)
    X = 1:YEARS
    for i = 1:YEARS
        calc_co2(s, CONSUMPTION_PER_YEAR*1e6)
...
X, Y = calc_scenario(16)
plot(X, Y, label="yearly CO2 emmissions [t]")
X1, Y1 = calc_scenario(0)
plot!(X, Y1, label="yearly emmisions without turbines [t]")